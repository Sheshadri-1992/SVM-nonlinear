...
D = 3
gammaList = [0.5,0.5,0.6,0.7,0.8,0.9]
kernelType = 'poly'
# gamma = 1
print kernelType
ck=1
for C in range(16,22):
	...
	for i in range(1,len(gammaList)):
		...
		for epochs in range(num_epoch):
			...
			for k in range(10):
				# print "here"
				ecpochAccuracy = ecpochAccuracy + k_cross_validation.prepare_train_test_data(samples_data_svm_format,k,10,x1,y1,x2,y2,C,D,gammaList[i],kernelType)
				print ecpochAccuracy
			# print "epoch ",epochs, " Accuracy ",ecpochAccuracy/float(10)
			accList.append(ecpochAccuracy/float(10))

		accList = np.array(accList)
		score_Matrix[ck][i]=accList.mean()
		sd_Matrix[ck][i]=np.std(accList)
